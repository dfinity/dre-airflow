<script lang="ts">
    ...
    import { type HostOsBatchDetail as HostOsBatchDetailType } from "../../../../../../../lib/types";
    ...
    async function fetchBatch(): Promise<HostOsBatchDetailType> {
        ...
        if (!r.ok) {
            throw new Error(`(code ${r.status}): ${await r.text()}`);
        } else {
            return await r.json();
        }
    }

    let promise = fetchBatch();
</script>
