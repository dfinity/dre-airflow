...
def rollout_planner_dag_run(
    rollout_schedule: dict[str, dict[str, List[int]]]
) -> Dict[str, Dict[str, datetime.datetime]]:

    context = get_current_context()
    week_plan = week_planner(context['logical_date'])

    res = dict()
    for dayname, hours in rollout_schedule.items():
        try:
            date = week_plan[dayname]
        except KeyError:
            raise ValueError(f"{dayname} is not a valid day")
        ...
        for time_s in hours.keys():
            ...
            res[dayname][time_s] = date.replace(hour=time.hour, minute=time.minute)
    
    for day, hours in res.items():
